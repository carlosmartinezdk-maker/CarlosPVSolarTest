...
## BALANCE-OF-PLANT TARGETING VIEW (gas only)

A second view, toggled by a **View** segmented control at the top: `Performance` / `BoP targeting`. Same map, chart and table components, repointed at the Part B outputs.

### Map
Dots **coloured by conviction tier**, sized by capacity:
```
CONFIRMED  #b0452f    LIKELY  #c47f45    EXPOSED  #c8b05a    LOW  #b9b5b1
```
Tooltip: plant, customer + SSI badge, capacity, BPRI, tier, summer penalty %, starts/MW/yr, trip rate, recoverable $, and the suggested inspection scope in one line.

### Filters — these are the prospecting controls
- **Tier** — All / Confirmed / Likely / **Exposed** / Low. Exposed on its own is the preventative-sale list.
- **BPRI band** — All / >80 / >60 / >40
- **Evidence vs exposure** — All / Evidence-led (C1 or C2 high) / Exposure-led (C3–C6 high, no current signal)
- **BoP age** — All / >15 yrs / >20 yrs since original COD
- **Cycling intensity** — All / heavy (starts per MW per year above fleet p75)
- **Confidence** — All / CEMS-backed only. Default this to **CEMS-backed only** in the targeting view; a cold call built on a proxy score is worse than no call.
- Plus the shared Customer, State, Relationship (SSI / prospect), Year filters.

### Table — two row modes specific to this view

**By site** — `Plant` · `Customer` + SSI badge · `St` · `MW` · `Original COD` · `BoP age` · `BPRI` · `Tier` · `Summer penalty %` · `Cooling trend %/yr` · `Starts/MW/yr` · `Trip rate` · `Recoverable $/yr` · `Confidence` · `Suggested scope`

**By customer — the prospect list.** This is the view that gets used most, so build it properly:

`Customer` · SSI badge · `Gas sites` · `Total gas MW` · `Sites confirmed` · `Sites likely` · `Sites exposed` · **`Share at risk %`** · `Capacity-weighted BPRI` · `Recoverable $/yr` · `Worst site` · `States` · `BAs`

Sort default on `Recoverable $/yr`, with a one-click re-sort on **`Share at risk %`** — and put a short note under the table explaining the difference, because it is the whole point:

> Recoverable $ ranks the biggest prize. Share at risk ranks the most systemic — a customer with eight of ten plants flagged has an O&M problem, not a plant problem, and that is a portfolio conversation with a different buyer.

Apply a minimum fleet size of 3 sites to the share-at-risk ordering so a single-asset owner at 100% does not top the list. Show the excluded small owners in a collapsed section rather than dropping them.

Clicking a customer row drills to their sites and repoints the map and chart, exactly as in the fleet version.

### Chart
Stacked columns of **capacity by tier over time** (or recoverable $ by tier), so a customer's BoP position visibly deteriorating or improving across years is readable at a glance.

### Export
A **Copy target list** button that puts the currently filtered customer or site table on the clipboard as TSV, ready to paste into a CRM or a call sheet. This is the one place where an export button genuinely earns its space.
...
- **Gas, Performance view** — sites · capacity · total recoverable $/yr · median HRI · % of capacity at SSI customers · MMBtu wasted
- **Gas, BoP targeting view** — sites in scope · capacity at risk · **% of filtered capacity at risk** · customers with ≥1 confirmed site · total recoverable $/yr · median BoP age
...
**Gas, BoP view:** the balance-of-plant index is a **propensity score, not a diagnosis** — it detects a system-level thermodynamic consequence (summer heat-rate penalty surviving ambient correction, consistent with condenser or cooling-water degradation), never a component fault. Measured detection floor is 4.5% heat rate change in a single month and 1.30% sustained over twelve; total failure of the largest auxiliary pump on a 500 MW plant is about 1.2% of output, so individual pumps are below the floor by construction. Confirming a cause requires an on-site condenser performance test and pump vibration survey. Rows marked low confidence have no CEMS data and use weak monthly proxies for cycling and trip exposure.
...
4b. BoP view: tier counts sum to the site count; `share_at_risk` per customer equals `(confirmed + likely) MW / total gas MW` recomputed from the site rows; the CEMS-backed filter changes the row count (if it does not, the confidence flag is not being set).
...
`gas_results_infographic.html` (both views — Performance and BoP targeting — in the one file, toggled) or `bess_results_infographic.html`, self-contained, under 16 MB. Present the file; do not publish it unless asked.
