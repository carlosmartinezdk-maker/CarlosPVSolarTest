...
Generated: 2026-08-12T13:40:21.223406
**SCOPE: 1788-site sample (CA (821), NC (769), SC (120), 25 other states), not the full 6,204-site fleet, per the brief's own BUILD ORDER (Section 12). Full 6,204-site scale-up is a tracked follow-up, not yet run.**

## NSRDB access status (READ FIRST)
Track A (NSRDB, PSM v4 GOES Aggregated at developer.nlr.gov) is reachable from this session's network egress policy - confirmed via `s2_nsrdb.py --preflight-only` (`success`) and by the real pull below. Per the brief's own updated architecture, `s2_nsrdb.py` is a fully standalone script with no import from the rest of this repo, run against 1788-site sample (CA (821), NC (769), SC (120), 25 other states), not the full 6,204-site fleet (9191 cell-years). The full 4,798-cell x 7-year = 33,586 cell-year fleet pull is still a separate, deliberately-deferred multi-day job at the NLR rate limit (1 req/sec, 10,000/day) - not yet run.

Weather source used this run: `{'A': 9191}` - 100% Track A (real NSRDB PSM v4 GOES Aggregated weather, 9191 cell-years) this run - the weather-track caveat is cleared. Still a validation-scale sample, not the full 6,204-site fleet.
...
- 1788 sites, stratified on tracking x module tech x years_present
- tracking mix: {'Fixed tilt': 944, 'Single-axis': 798, 'Dual-axis': 26, 'Unknown': 14, 'Mixed': 6}
- tech mix: {'crystalline_silicon': 1590, 'cdte': 183, 'unknown': 15}

## S4 - PI/PRI coverage
- 159132 site-months scored: 96062 peer-benchmarked PRI, 63070 physical-fallback (PI_adj) - 60% peer coverage (Explorer v2 brief A3 predicted this would improve with geographically clustered sampling - it did: 60% here vs. 12.6% on the earlier scattered 100-site nationwide subsample)
- PI distribution: {'count': 104802.0, 'mean': 0.842, 'std': 0.331, 'min': -2.161, '25%': 0.751, '50%': 0.879, '75%': 0.963, 'max': 23.265}

## Explorer v2 brief, Part A2 - PI bias diagnostic
- Regression of monthly PI on month-of-year and latitude (n=104802): latitude coefficient **0.00149** (p=0.0127), PI by calendar month: `{1: 0.805, 2: 0.845, 3: 0.855, 4: 0.855, 5: 0.857, 6: 0.864, 7: 0.863, 8: 0.864, 9: 0.847, 10: 0.834, 11: 0.824, 12: 0.798}`.
...
- **A4 - SNOW rate by state (top 10 of 16 states with classified months):** `{'WI': 0.51, 'UT': 0.47, 'MI': 0.46, 'MN': 0.39, 'RI': 0.37, 'ME': 0.33, 'OR': 0.3, 'CO': 0.29, 'VT': 0.28, 'IL': 0.27}` - elevated rates in states without heavy winter snowfall (e.g. NJ, KY, UT) are consistent with the G2 gate partly firing on the same PI winter bias documented above, not solely on real snow. Not yet separated from genuine snow events at subsample scale.

## TEST 11 - Degradation sanity (the best end-to-end check available)
- Median beta_excess at 4+ years of history: **-0.0090** (target ~ -0.0050, i.e. -0.5%/yr)
- Caveat: fit against real Track A NSRDB weather, 1788 sites total, 1782 clearing the 4-year decision-grade bar. Re-run at full scale (4,396 sites with 4+ years) before trusting this number as fleet-wide.
...
- BLOCK_OUTAGE: Var/mean = 0.09 - BOS_INTERMITTENT: Var/mean = 0.20 - OUTAGE_FULL: Var/mean = 0.11 - SOILING: Var/mean = 0.08 - TRACKER: Var/mean = 0.13 - UNATTRIBUTED: Var/mean = 0.18 
## TEST 22 - Classifier profile sanity (episodes per plant-year)
| signature | this run | reference (Part 7 Step E1) |
|---|---|---|
| UNATTRIBUTED | 0.26 | 0.76 |
| BOS_INTERMITTENT | 0.30 | 0.58 |
| SOILING | 0.06 | 0.31 |
| BLOCK_OUTAGE | 0.19 | 0.29 |

At n=1788 sites, deviation from the reference is still worth reading with caution (sample composition, not weather-track) but is far more informative than at 100-site scale - re-check at full fleet scale before treating it as final.

## S9 - Dollars (Section 0.2)
- PRI_P75 (this fleet, 96062 peer-benchmarked PRI values): **1.0134**
- Part 8 vs tracker quick-estimate cross-check: 93% of 32502 dollar-bearing site-months within 15% of each other. Per the brief, a wide gap points at a peer-set problem, not a rounding difference - and with only 2% peer coverage at subsample scale, that is exactly what this low agreement rate reflects. Expect much tighter agreement at full scale.

## Known limitations this run (encoded as flags, not silently absorbed)
- Real Track A NSRDB weather this run (see above) - the weather-track caveat is cleared; 1788-site sample scale (not the full 6,204-site fleet) is now the single biggest caveat on every number.
...
Written to `output/call_list_subsample.csv` (35553 dollar-bearing site-months total).
